...
Which match is taken matters more than it looks. The player requests the **master** playlist first and then one playlist per track it intends to use, and the per-track ones carry a `type=` selector (`type=video`, `type=audio`, `type=subtitle`). Titles are not encoded consistently: on some, audio is muxed into each video rendition, but on others it is a separate `type=audio` rendition that the master exposes through an `EXT-X-MEDIA` group. Keeping a later match therefore risks handing yt-dlp a video-only playlist, which downloads into a perfectly good but completely silent file. The listener guards against this twice — it ignores everything after the first match, and it skips any URL carrying a `type=` selector — so what reaches yt-dlp is always the master, which lists every track and lets yt-dlp pair video with audio and merge them.
...
- On titles whose audio is a separate rendition, yt-dlp downloads video and audio in two passes, so the shelf's progress bar runs to 99% and then restarts from 0 for the (much shorter) audio pass. Cosmetic only, but it looks like a stall.
